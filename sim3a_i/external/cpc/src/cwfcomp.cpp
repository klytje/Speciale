...
{ 
  ...
  for (unsigned int i = N_num-1 ; i <= N_num ; i--)
  {
    ...
    if (!isfinite (H) || !isfinite (dH)) cout<<"Numerical failure encountered in H_dH_direct_integration."<<endl;//,exit (1); XXX
...
void Coulomb_wave_functions::F_dF_direct_integration (const complex<double> &z,complex<double> &F,complex<double> &dF,bool &is_it_successful)
{
  ...
  for (unsigned int i = N_num-1 ; i <= N_num ; i--)
  {
    ...
    if (!isfinite (F) || !isfinite (dF)) cout<<"Numerical failure encountered in F_dF_direct_integration."<<endl;//,exit (1); XXX
  }
  is_it_successful = true;
}
...
{  
  ...
  if (!isfinite (F) || !isfinite (dF)) cout<<"Numerical failure encountered in F_dF."<<endl;//,exit (1); XXX
...
void Coulomb_wave_functions::H_dH (const int omega,const complex<double> &z,complex<double> &H,complex<double> &dH)
{
  ...
  else
  {
    ...
    { 
      ...
      //if (!neg_int_omega_one && !neg_int_omega_minus_one) H_dH_with_expansion (omega,z,H,dH,is_it_successful),cout << "h"<<endl;
  ...
  if (!isfinite (H) || !isfinite (dH)) cout<<"Numerical failure encountered in H_dH."<<endl;//,exit (1); //XXX
}
...
{ 
  ...
  if (!isfinite (H_scaled) || !isfinite (dH_scaled)) cout<<"Numerical failure encountered in H_dH_scaled."<<endl;//,exit (1); //XXX
}
