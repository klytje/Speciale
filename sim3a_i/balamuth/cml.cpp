...
void checkSpinParity(int J1, int J2, int p1, int p2, int L) {
    auto pi = p1;
    auto pf = p2 * pow(-1, L);
    if (pi != pf) {
        cerr << "Parity violation! " << pi << " != " << pf << "  (" << p2 << " * [-1]^" << L << ")" << endl;
        exit(4);
    }

    if (!(abs(L - J2) <= J1 && abs(L + J2) >= J1)) {
...
CommandLineOptions parseCml(int argc, char **argv) {
    ...
    cml.interference = false;

    cml.N = 100000;

    cml.J1 = cml.J2 = cml.L = cml.L2 = -1;
    cml.p1 = cml.p2 = 0;
    cml.r0p = 1.87;
    cml.r0s = 1.42;
    cml.gs = NAN;
    ...
    checkSpinParity(cml.J1,cml.J2,cml.p1,cml.p2,cml.L);

    if (cml.interference) {
        if (cml.L2 < 0) {
            cerr << "When using interference, L2 must be > 0! (or equal?)" << endl;
            exit(1);
        }
        checkSpinParity(cml.J1,cml.J2,cml.p1,cml.p2,cml.L2);
    }
