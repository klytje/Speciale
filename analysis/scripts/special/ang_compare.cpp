...
int main(int argc, char const *argv[]) {
    ...
    double bounds[2] = {0.3, 0.5}; // y axis bounds
    ...
    auto ang_corr = [] (Double_t* xx, Double_t* par) {
        double x = xx[0]*3.5;
        double res = 11.25*pow(cos(x), 4) - 7.5*pow(cos(x), 2) + 1.25;
        return 200*res;
    };

    TF1 *corr = new TF1("corr", ang_corr, -1, 1);
    corr->SetParameters(0);
    corr->Draw("same");
