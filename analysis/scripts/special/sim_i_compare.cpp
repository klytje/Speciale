...
int main(int argc, char *argv[]) {
    if (argc != 4) {
        cout << "Usage: ./sim_i_compare <output path> <output/X.root> <comparison file>" << endl;
        exit(1);
    }
    ...
    TH1D sim_rho = dsim.Define("x", "sqrt(pow(X,2)+pow(Y,2))").Histo1D({"sim_rho", "sim_rho", int(x_axis[0]), x_axis[1], x_axis[2]}, "x").GetValue();
    ...
    TH1D sim_ang = dsim.Define("x", "atan2(X,Y)").Histo1D({"sim_ang", "sim_ang", int(x_axis[0]), x_axis[1], x_axis[2]}, "x").GetValue();
    ...
    for (int i = 0; i < 3; i++) {
        TH1D sim_temp = dsim.Define("x", (format("E_cm[%1%]") % i).str()).Histo1D({"sim_temp", "sim_temp", int(x_axis[0]), x_axis[1], x_axis[2]}, "x").GetValue();
