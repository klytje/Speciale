...
int main(int argc, char *argv[]) {
    ...
    // set the axes    
    double x_axis[] = {400, 7.5e3, 19e3};
    double y_axis[] = {400, 0, 250e3};

    //*** PLOT ***//
    setup_style();

    TCanvas* canvas = new TCanvas("ExpT", "ExpT", 600, 600);
    TH2D hist = df.Histo2D({"h2", "ExpT", int(x_axis[0]), x_axis[1], x_axis[2], int(y_axis[0]), y_axis[1], y_axis[2]}, "exC12", "p_tot").GetValue();
