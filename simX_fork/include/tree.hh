...
// Copyright (C) 2015 Michael Munch <munch@phys.au.dk>
// Possible to use tree with move only types (std::unique_ptr)
...
class tree {
	...
	public:
      ...
      class leaf_iterator;
		...
		tree(T&&);                   	                // constructor setting given element as head
      ...
      class leaf_iterator : public iterator_base {
		...
		/// Return fixed-depth iterator to the first node at a given depth from root.
		fixed_depth_iterator begin_fixed(unsigned int) const;
      ...
      leaf_iterator   end_leaf(const iterator_base& top) const;
		...
		template<typename iter> iter insert(iter position);
...
template <class T, class tree_node_allocator>
tree<T, tree_node_allocator>::tree(T&& x)
{
	head_initialise_();
	set_head(std::move(x));
}
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::fixed_depth_iterator tree<T, tree_node_allocator>::begin_fixed(unsigned int dp) const {
	return begin_fixed(begin(), dp);
};
...
iter tree<T, tree_node_allocator>::append_child(iter position, const T& x)
	{
	auto tmp = append_child(position);
	tmp.node -> data = x;
	return tmp;
	}
...
iter tree<T, tree_node_allocator>::prepend_child(iter position, const T& x)
	{
		auto tmp = prepend_child(position);
		tmp.node -> data = x;
		return tmp;
	}
...
template <class T, class tree_node_allocator>
bool tree<T, tree_node_allocator>::iterator_base::isLeaf() const
{
	tree_node *pos=node->first_child;
	return pos == nullptr;
}
